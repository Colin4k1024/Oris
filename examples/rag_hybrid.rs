...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use oris::vectorstore::{pgvector::PgOptions, VectorStore};
    let _ = store
        .add_documents(&documents, &PgOptions::default())
        .await?;

    // Create retriever
    let retriever: Arc<dyn oris::schemas::Retriever> =
        Arc::new(Retriever::new(store, 3));

    // Create LLM
    let llm = OpenAI::default().with_model(OpenAIModel::Gpt4oMini.to_string());
    let llm_box1: Box<dyn oris::language_models::llm::LLM> = Box::new(llm.clone());
    let llm_box2: Box<dyn oris::language_models::llm::LLM> = Box::new(llm.clone());
