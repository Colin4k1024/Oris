...
第六批已落地：`evolution-network` 标准入口收窄为 protocol/facade 能力；`/v1/evolution/*`、`/evolution/a2a/*`、session replication/lifecycle 等宽 HTTP 路由迁移到显式 `evolution-network-routes`。旧 `evolution-network-experimental` 与 `full-evolution-experimental` 仍会启用该路由组以保持兼容。
...
| `evolution-network` | `evokernel-facade`, `evolution` | 标准 network protocol/facade；不暴露宽 HTTP 路由 |
| `evolution-network-routes` | `evolution-network` | 显式实验宽路由门禁 |
...
| `evolution-network-experimental` | `evolution-network-routes` | 旧兼容入口 |
